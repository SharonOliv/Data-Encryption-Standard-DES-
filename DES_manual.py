...
SHIFT_SCHEDULE=[1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1] # Number of left shifts for each round
...
def pad_text(text): #Pad text to be multiple of 8 bytes
    padding_length = 8 - (len(text) % 8)
    return text + " " * padding_length

def pad_key(key): #Ensure key is exactly 64 bits
    if len(key) < 8:
        return (key + "0" * 8)[:8]
    return key[:8]
...
def binary_to_hex(binary): #Convert binary string to hexadecimal
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print("\nEncrypting...")
            ciphertext = des_encrypt(text_binary, key)
            print("Encrypted (HEX):", binary_to_hex(ciphertext))
        elif choice == '2':
            hex_input = input("\nEnter the hex ciphertext: ")
            binary = hex_to_binary(hex_input)
            print("\nDecrypting...")
            decrypted_binary = des_decrypt(binary, key)
